...
inline static void register_semantier(AstMap &asts, const string &ast_folder) {
    ...
    load_heartbeatAst(asts, (folder_path / "ast/heartbeat.ast.json").string());
    load_ansessionreqAst(asts, (folder_path / "ast/session-req.ast.json").string());
    load_ansessionrespAst(asts, (folder_path / "ast/session-resp.ast.json").string());
}
